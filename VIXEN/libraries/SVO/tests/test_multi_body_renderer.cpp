...
TEST(CelestialRenderListBuilder, BuildsPerBodyLodAndSortsNearToFar) {
    ...
    EXPECT_EQ(list.items[0].regime, FootprintRegime::System);
    EXPECT_EQ(list.items[1].regime, FootprintRegime::Orbital);
...
TEST(CelestialRenderListBuilder, RegimeFloorAppliesIndependentlyPerBody) {
    ...
    EXPECT_EQ(nearBody.regime, FootprintRegime::SurfaceDetail);
    EXPECT_EQ(farBody.regime, FootprintRegime::DeepField);
