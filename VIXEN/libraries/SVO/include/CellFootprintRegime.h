#pragma once
// CellFootprintRegime.h — E6-T1: shared CPU/GPU footprint classifier (stencil doc's
// prerequisite slice; residency-unification design's step 2).
//
// This is the CPU twin of shaders/SceneBindings.glsl's classifyCellFootprintRegime (defined
...
enum class CellFootprintRegime : unsigned int {
    Surface = 1,
    MipHit  = 2,
    Cosmic  = 3,
};

// Matches shaders/SceneBindings.glsl's classifyCellFootprintRegime brick-cell subdivision
...
inline CellFootprintRegime ClassifyCellFootprintRegime(float worldDist, float cellWorldSize,
    ...
    if (raySizeCoef <= 0.0f || footprint < cellWorldSize / kBrickDivisor) {
        return CellFootprintRegime::Surface;
    }
    if (footprint < cosmicK * cellWorldSize) {
        return CellFootprintRegime::MipHit;
    }
    return CellFootprintRegime::Cosmic;
}
