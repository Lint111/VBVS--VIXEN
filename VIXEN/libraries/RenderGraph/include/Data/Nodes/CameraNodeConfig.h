...
CONSTEXPR_NODE_CONFIG(CameraNodeConfig,
    ...
    // Full camera position as one Vec3 (spec 2026-07-04 camera-transform-refactor — replaces the
    // old PARAM_CAMERA_X/Y/Z scalar triple). Applied via SetPosition onto `transform` directly.
    static constexpr const char* PARAM_CAMERA_POSITION = "camera_position";
    ...
    // One Vec3 (spec 2026-07-04 camera-transform-refactor — replaces the old X/Y/Z float triple).
    static constexpr const char* PARAM_ORBIT_CENTER = "orbit_center";
    ...
    // Initial free-fly pose request (spec 2026-07-04 camera-transform-refactor — replaces the
    // 5-scalar PARAM_INITIAL_FLY_X/Y/Z/YAW/PITCH from an earlier iteration with ONE Mat4). The
    // host computes a scene-scale-aware starting position/orientation (from the same RenderWindow
    // used to frame the voxel-grid section) via ComposeTransform and sends it once at boot, or
    // again on the `resetcam` console command. Same pose-param semantics: applied via a
