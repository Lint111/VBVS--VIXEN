...
void GraphicsPipelineNode::CreatePipelineCache() {
    ...
    // Request tracked allocation for pipeline cache
    auto* rm = GetResourceManager();
    if (rm) {
        pipelineCache_ = rm->RequestSingle<VkPipelineCache>(
            AllocationConfig()
                .WithLifetime(ResourceLifetime::GraphLocal)
                .WithName("pipelineCache")
                .WithOwner(GetInstanceId())
        );
        if (pipelineCache_.IsError()) {
            throw std::runtime_error("Failed to allocate pipelineCache: " +
                std::string(pipelineCache_.GetErrorString()));
        }
    }
    ...
    VkResult result = vkCreatePipelineCache(
        ...
        &cacheInfo,
        nullptr,
        pipelineCache_.GetPtr()
    );

    if (result != VK_SUCCESS) {
        throw std::runtime_error("Failed to create pipeline cache");
    }
    pipelineCache = pipelineCache_.Get();  // Keep legacy handle in sync
}
