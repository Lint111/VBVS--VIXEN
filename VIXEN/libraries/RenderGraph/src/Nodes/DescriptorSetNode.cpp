...
void DescriptorSetNode::ExecuteImpl(TypedExecuteContext& ctx) {
    ...
    if (!writes.empty()) {
        ...
        NODE_LOG_DEBUG("[DescriptorSetNode::Execute] No Execute writes for frame " + std::to_string(imageIndex));
    }
}
...
std::vector<VkWriteDescriptorSet> DescriptorSetNode::BuildDescriptorWrites(
    ...
    NODE_LOG_DEBUG("[BuildDescriptorWrites] Building writes for roleFilter=" + std::to_string(static_cast<uint8_t>(roleFilter)) +
                  " (Dependency=" + std::to_string(static_cast<uint8_t>(roleFilter) & static_cast<uint8_t>(SlotRole::Dependency)) +
                  ", Execute=" + std::to_string(static_cast<uint8_t>(roleFilter) & static_cast<uint8_t>(SlotRole::Execute)) +
