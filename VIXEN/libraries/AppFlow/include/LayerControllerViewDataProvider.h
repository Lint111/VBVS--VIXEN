...
// Day-one direct-field IViewDataProvider (seam doc's "~3 lines"): noun LayerMask <->
...
public:
    ...
    bool ReadU32(ViewNounKey key, uint32_t& out) const override {
        if (key.noun != ViewNounId::LayerMask) return false;
        out = layers_.Mask();
        return true;
    }

    void WriteU32(ViewNounKey key, uint32_t value) override {
        if (key.noun != ViewNounId::LayerMask) return;
