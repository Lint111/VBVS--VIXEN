...
## 10. Addendum: December 28, 2025 - Timing Bug Fix

### 10.1 Issue Discovered

A critical timing bug was discovered in benchmark data collected before December 28, 2025.

**Symptoms**:
- Compute pipeline showed unrealistically fast times (similar to Fragment/HW_RT)
- All pipeline frame times were elevated compared to corrected measurements
- Cross-pipeline comparisons were invalid due to inconsistent timing

**Root Cause**: Frame timing measurement was affected by synchronization issues in the benchmark framework.

### 10.2 Data Comparison

| Pipeline | OLD Data (Pre-Dec 28) | NEW Data (Fixed) | Change |
|----------|----------------------|------------------|--------|
| compute | 10.23ms, 230 FPS | 8.00ms, 291 FPS | Fixed timing |
| fragment | 6.35ms, 657 FPS | 2.78ms, 657 FPS | 2.3x faster (corrected) |
| hardware_rt | 10.84ms, 776 FPS | 1.89ms, 1033 FPS | 5.7x faster (corrected) |

### 10.3 Key Findings

1. **Compute is correctly the slowest pipeline** - This is expected behavior for a pure compute approach without rasterization/RT hardware
2. **Fragment uses GPU rasterization hardware** - Showing 2.78ms frame times
3. **Hardware RT uses dedicated RT cores** - Showing 1.89ms frame times (fastest)

### 10.4 Dataset Impact

**Research Dataset v2** (`benchmarks_research_v2.xlsx`):
- Uses ONLY corrected December 28 data
- Sample sizes are smaller due to limited re-testing:
  - Compute: 117 samples (vs 505 in old data)
  - Fragment: 113 samples (vs 496 in old data)
  - Hardware RT: 95 samples (vs 584 in old data)

**Recommendation**: All research analysis should use `benchmarks_research_v2.xlsx` which contains only validated timing data.

### 10.5 Files Affected

- Old biased data: `data/benchmarks.xlsx` (DEPRECATED - do not use for pipeline comparisons)
- Fixed data: `data/benchmarks_research_v2.xlsx` (USE THIS)
- Fixed data location: `data/benchmarks/VIXEN_Benchmarks_20251228_compute_fixed/`

### 10.6 Research Finding: GPU Type Performance Ratios

Analysis of corrected data reveals significant differences in pipeline performance ratios between GPU types:

| GPU Type | Fragment/Compute | HW_RT/Compute | Interpretation |
|----------|-----------------|---------------|----------------|
| RTX 3060 (Dedicated) | **9.2x** faster | **12.5x** faster | Dedicated GPUs massively benefit from hardware acceleration |
| AMD Radeon (Integrated) | **1.1-1.5x** faster | **2.0-2.3x** faster | iGPUs show smaller gains, compute is more competitive |

**Key Insights**:
1. **Dedicated GPUs**: Fragment and HW_RT pipelines leverage dedicated rasterization hardware and RT cores, resulting in 9-12x speedup over pure compute
2. **Integrated GPUs**: Limited shader cores and shared memory reduce the advantage of specialized hardware paths
3. **Compute Pipeline Competitiveness**: Pure compute approach is relatively more viable on resource-constrained iGPUs

**Research Implication**: Pipeline selection should consider target hardware class. Compute-based voxel rendering may be preferred on integrated graphics where the overhead of hardware pipelines outweighs their benefits.

### 10.7 Note on Old Dataset

The pre-December 28 dataset (`benchmarks.xlsx`) cannot be used for cross-GPU pattern analysis due to inconsistent timing measurements. The old data showed anomalies such as:
- AMD iGPU appearing faster than RTX 4080 (clearly incorrect)
- Same GPU showing 3x variance across runs
- Inconsistent pipeline rankings within same hardware

**Conclusion**: Only use `benchmarks_research_v2.xlsx` for research. Additional GPU testing with the corrected benchmark framework is recommended to expand the cross-GPU dataset.
...
*Addendum Date: December 28, 2025*
