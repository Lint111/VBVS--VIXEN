...
- Milestone 5 (Task 10 + live gate): **PARTIAL — Stored-SDF path renders, but a traversal artifact remains; the correct redesign is identified.** Task 10 (Stored demo seed + node bake-path) commit `bf73cda8`; live runs `bjm41yng3`/`bs5vu4dpq` (0 syncval). **Result: the Stored-SDF iso-surface RENDERS — clean smooth spheres + a displaced sphere are visible on screen** (trilinear iso-surface, gradient normal, cross-brick lookup, descriptor, bindings 11/12, and the `renderScale`/grid-voxel scale-coupling all work). Remaining artifact: POV/angle/distance-dependent brick-aligned holes.
- **Root cause (live-gate, 2026-06-22):** the standalone `marchStoredSdf` is a FLAT sphere-trace that treats an unallocated narrow-band brick as a *distance* (`+1e9`) and lunges `t += (1/√3)·1e9` → skips bricks / the whole body / lands in interior gaps. It reinvents — badly — the octree "graph movement" that `ESVOTraversal.glsl` already does. Live iterations (narrow-band→dense, then `1/√3` Lipschitz step `t += max(d·0.5773503, EPS)` + MAX_STEPS 256) shrank but never closed it → confirmed a **traversal** bug, not data shape. The dense-field debug hack was reverted; the data is honest narrow-band.
