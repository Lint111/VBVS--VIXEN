...
- **M4 ✅ DONE — Demo graph (Tasks 7–8).** `BuildAutoSyncDemoGraph` (`compute→compute→render→present`, fullscreen frag reads compute SSBO) + `VIXEN_AUTOSYNC_DEMO` dispatch + shaders. *Gate (agent):* full build green (shaders compile at RUNTIME via `ShaderLibraryNode` — no build-time SPIR-V step; construct/render/syncval verified at M5).
- **M5 ✅ DONE — Live gate (Task 9).** Demo renders the compute gradient + **0 synchronization-validation errors**; main-app regression clean. *The live gate caught 4 bugs (2 core engine) that 3 static reviews missed — see Progress Log.*
...
- Milestone 4 (Tasks 7–8): DONE · commits `cf3d206e`, `8aa17a93`, fix `e0740fc1` · Opus validator: 1 fix-loop — caught a NON-DETERMINISTIC compile-ordering hole (`COMPILE_AFTER` wired from only 1 of 9 handle sources; `TopologicalSort` falls back to pointer order for graph-independent nodes → `assert(!passes_.empty())` could abort or render blank by allocator/run) → fixed by making `PassGroupNode` a `VariadicTypedNode` wired from ALL 9 handle sources (`VariadicConnectionRule` adds real topology edges) = true topological guarantee — then APPROVED (build 82/82 green, both tests 3/3, clean tree, no `.claude`/doc leakage). **Approved pure-path deviations vs plan:** handle resolution via `RegisterPostNodeCompileCallback` (build-time handles unavailable); **3 descriptor sets / same `VkBuffer`** (shared compute+fragment layout impossible — `DescriptorSetNode` reflection-only, layout-spec path dead); new generic `StorageBufferNode`; **shaders compile at runtime** (no build-time SPIR-V). · M4 agent-gate = build green; M5 live gate done (below) · 2026-06-21
- Milestone 5 (Task 9, live gate): DONE — **the live gate earned its keep.** First run ASSERTED (`passes_` empty) — the very bug 3 static reviews had "fixed"/approved but never executed. Live instrumentation root-caused 4 distinct bugs: **(core)** `RenderGraph::GeneratePipelines` fired post-compile callbacks before `SetState(Compiled)`, so the demo's aggregation callback never assembled `passes_`; **(demo)** `DescriptorSetNode::DESCRIPTOR_SETS` read via the wrong type tag → empty sets → `VUID-08600`/device-lost; **(demo)** render-pass `General`+LOAD vs the Undefined swapchain → per-frame `VUID-09600` → `Undefined`+`DontCare`; **(core)** `ComputePipelineNode` keyed its pipeline cache on the *descriptor-interface* uuid (code-agnostic) → the two same-interface compute shaders shared ONE pipeline → `post` never ran → flat gray → re-keyed on the SPIR-V **code** hash. Both core changes Opus-validated SAFE globally (only 1 post-compile-callback consumer exists; the pipeline re-key is strictly-more-correct de-aliasing) with **main-app regression clean** (renders, 0 syncval). Demo now renders the gradient (visual proof) + 0 syncval; 57/57 tests green. Commits `d4dc4f96`, `a39a24be`. **Lesson: static validation repeatedly passed bugs that only the live run caught — the live gate is authoritative.** · 2026-06-22
