...
- M3b-3 (Task 3): DONE · Yeroket `407d4258` + VIXEN `34bab410` · Opus validator APPROVED (Pyramid PNG = proper single pyramid, verified vs an INDEPENDENT IQ port max|diff|=0; RoundCone tamper-confirmed 3-region) · dotnet 91/95, parity 33/33, codegen 6/6, bake 1/1, RenderPyramid 42,242px, all prior renders non-regressed · 2026-06-27. 6 prism/cone leaves; RoundCone branch→3-region-`select` rewrite; Segment no-position (pointA/pointB carry the geometry); generated enum now 37 members. **2 fix-loops on Pyramid + a user-approved CANONICAL BUGFIX:** (1) abs-all vs canonical xz-only; (2) the swap swizzle was Y↔Z where IQ `sdPyramid` is X↔Z — `SDFPrimitives.cs:553` `q.xzy→q.zyx` (a genuine bug in canonical Yeroket source; fix also corrects Unity's Pyramid node). **DURABLE GOTCHA (the big one):** BOTH Pyramid bugs passed parity 33/33 because the oracle was re-derived from the SAME buggy canonical → CIRCULAR validation. Only the live-render PNG read + an INDEPENDENT IQ-formula port caught them. **RULE for catalogue-fill: validate primitive SHAPE against an EXTERNAL reference (IQ formula / geometric first-principles / rendered silhouette), NOT just kernel↔oracle parity** — a parity oracle transcribed from the same source the kernel copies cannot catch a shared transcription bug.
