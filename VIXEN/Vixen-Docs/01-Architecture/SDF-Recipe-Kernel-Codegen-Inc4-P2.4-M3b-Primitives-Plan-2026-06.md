...
- M3b-3 (Task 3): DONE · Yeroket `7b0e3b11` + VIXEN `07fc5fbf` · final review APPROVED (20/20 batch) · dotnet 91/95, parity 33/33, codegen 6/6, bake 1/1, RenderPyramid 18,802px, all prior renders non-regressed · 2026-06-27. 6 prism/cone leaves; RoundCone branch→3-region-`select` rewrite; Segment no-position (pointA/pointB carry the geometry); generated enum now 37 members. **Pyramid took THREE fix-loops + a user-approved CANONICAL BUGFIX of `SDFPrimitives.cs` (a garbled IQ `sdPyramid` transcription; the fix also corrects Unity's Pyramid node):** (1) abs-all → canonical xz-only; (2) swap swizzle Y↔Z → IQ's X↔Z (`q.xzy→q.zyx`); (3) the squared-distance CORE was wrong (apex at y=2.0 not 1.0, read as inside; 3468/4352 grid pts off vs IQ) — replaced with the published-IQ `da`/`db`/gate/`sign` form (`SDFPrimitives.cs:560-567` + the kernel mirror). **DURABLE GOTCHA (the big one):** all 3 bugs passed parity 33/33 because the oracle was re-derived from the SAME garbled canonical → CIRCULAR validation; bug (3) ALSO passed the live-render PNG read (the silhouette looked pyramid-ish while 2× too tall). Only an INDEPENDENT published-IQ port over a 4352-point GRID (numerical max|diff|, apex y-crossing) caught it. **RULE for catalogue-fill: validate primitive SHAPE NUMERICALLY against an EXTERNAL reference (published IQ / geometric first-principles over a grid) — NOT kernel↔oracle parity (circular) and NOT a silhouette PNG alone (blind to a 2× scale error).** The final whole-batch review independently grid-checked all 20 kernels vs an IQ port: 19 passed first time, Pyramid after the core fix (max|diff|=0 over the grid).
- **M3b COMPLETE (20/20):** all leaf primitives wired + INDEPENDENTLY shape-verified (not just parity). Excluded: BezierCurve (nested loops), ProfileExtrude (runtime array). Flagged: CylinderRounded (no `SDFOpCode` member — needs an append-only enum addition to wire). Whole-batch final review APPROVED 2026-06-27. Generated enum: 37 members. Next = M4 (domain transforms + value/float3 math + non-uniform DistScale + `UPDATE_GOLDENS` regen-write mode), then M5 (broad live CSG-composition render gate).
