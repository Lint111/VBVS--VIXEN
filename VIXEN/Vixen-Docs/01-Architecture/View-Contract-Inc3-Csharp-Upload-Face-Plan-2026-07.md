...
- Milestone M1 (Tasks 1–2): DONE · Yeroket commits 89be0e44..fe2457af · Opus validator APPROVED (re-ran 25/25 suite) · 2026-07-07 · note: `[View]`/`[ViewSection]` live in `$KF/Runtime/GpuStructAttributes.cs` (ns `Yeroket.Util.KernelFramework`), not the plan's speculative Transpiler path — M3's `Hud.cs` resolves via the existing `using`.
- Milestone M2 (Tasks 3–5): DONE · Yeroket commits e7365931 (T3 ViewWireFormat), 820385e7 (T4 ViewWriterEmitter), be2d1a6e (T5 golden+Soa tests) · Opus validator APPROVED (hand-derived the 85-byte UTVA buffer from spec §4, re-ran FNV→0x55D27B8C independently, proved golden test non-vacuous via byte-[12] perturbation, 28/28 suite) · 2026-07-07 · note: M2 spanned two worker sessions (first cut off by API error mid-Task-4; second resumed from Task-4 verification) — final state coherent. Cross-language UTVA wire contract proven sound; M4's C++ CanonicalWire must reproduce the same 85 bytes.
- Milestone M3 (Tasks 6–7): DONE · Yeroket commit 31850594 (T6 --view-writer CLI); VIXEN-worktree commits 06c9e31b (T6 [ViewSection(Aos)] Hud.cs + generated Hud.view.g.cs), 33fd0a52 (T7 view_hud_writer_check/regen CMake pair) · Opus validator APPROVED (regenerated Hud.view.g.cs → cmp byte-identical to committed; tool builds clean; --check bites; token-level CMake structural read matches view_hud_blob_* verbatim) · 2026-07-07 · note: drift-guard verified via the direct WSL `--check` command (plan's documented fallback) rather than a full ~500s Windows reconfigure — CMake pair structurally verified not executed. Generated Hud.view.g.cs SchemaVersion 0x55D27B8C, six fields. VIXEN work is in the WORKTREE `feat/view-contract-inc3`.
- Milestone M4 (Tasks 8–10): DONE · VIXEN-worktree commits a9204e21 (T8 ViewWireReader.h/.cpp + RenderGraph CMake), 3b61efe2 (T9 test_view_wire_roundtrip.cpp + tests CMake) · Opus validator APPROVED (independently rebuilt Windows-side; 3/3 round-trip PASS; proved non-vacuous via tick 42→43 perturbation-fail-then-revert; verified §5.3 version-guard early-return is BEFORE any store write; bounds-checking sound; test_view_blob_equiv 3/3 + Yeroket 28/28 no-regression; both drift-guards passed on fresh configure) · 2026-07-07 · notes: (1) test registered mirroring test_view_hud_golden (same-dir sibling), NOT the plan's test_view_blob_equiv (which links heavier VixenApp) — sound substitution. (2) Malformed-BODY case partially mutates store before returning false — matches spec's ViewBlobFile::Parse reference; "no partial write" is the caller-boundary contract (false ⇒ don't flush); version-mismatch (the dangerous case) is fully early-guarded. Not a defect. (3) ViewWireReader compiles into RenderGraphNodes.lib. **ALL 4 MILESTONES DONE — Inc-3 complete.**
- Final whole-diff review (Opus, both repos): **APPROVED FOR MERGE** · 2026-07-07 · confirmed the C#↔C++ seam holds end-to-end (declared-order divergence closed by construction — both faces consume the same `ViewModelBuilder.Build`), names match across all 4 artifacts, version hash layout-independent single-source, no dead SoA scaffolding, D1–D7 + §8 all realized, cross-repo coherence proven via `--view-writer --check` (committed artifact byte-identical to committed emitter). 4 minor non-blockers noted. Follow-up commit `3ad08519` (VIXEN worktree): tightened `ViewWireReader.h` failure-contract comment (malformed-body may partial-write; `false`=don't-flush) per review minor #1.
- **Historical final state:** VIXEN worktree `feat/view-contract-inc3` closed at `3ad08519` (5
  commits off main `6cb9389b`); Yeroket `main` closed at `31850594` (6 commits, in-place). The
  VIXEN implementation is now present in the current engine history; the former push/merge gate
  is historical (`3ad08519` is an ancestor of the current branch).
