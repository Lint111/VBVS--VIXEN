...
- **Inc 2 (designed — `AppFlow-Framework-Inc2-Design-2026-07.md`):** `LayerController` (owns the layer enabled-mask `LayerState`, source of truth — NOTE: editor layers = document RE-FLATTEN via enabled-mask, NOT RenderGraph node-enable; the earlier "node enable" phrasing was wrong) + generic snapshot-fallback ActionStack engine (both undo paths + inverse-vs-snapshot parity + grouping) + the re-flatten seam. Live gate = HEADLESS GPU toggle→re-flatten→render→undo render-gate (extends `test_editor_document_render.cpp` bore-column pixel-diff). Windowed `EditorApplication` rewire split out to **Inc-2b**.
- **Inc 2b:** windowed `EditorApplication` rewire — retire hand-wired `DrainClickedElementId`/`ParseLayerToggleId`/`glfwGetKey(S)`/`ConsumeDirty` for AppFlow BindingStore+ActionStack, verified running the windowed editor.
