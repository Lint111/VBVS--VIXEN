...
- [x] **C2 — shader consumes the generated struct + live gate** · Task 2. `#include "Generated/OctreeConfig.glsl"` in BodyInstanceRayMarch.comp; drop the inline struct. Gate: `test_body_instance_raymarch_render` live no-regression on lavapipe + `test_octree_config_sdi_parity` green.
...
- Milestone C2 (Task 2): DONE · VIXEN commit `1bdf8ac4` (`shaders/BodyInstanceRayMarch.comp`: inline `struct OctreeConfig` → `#include "Generated/OctreeConfig.glsl"`, +1/-26) · gate (controller-run): live lavapipe render `test_body_instance_raymarch_render` **6/6** (identical to the pre-migration baseline I captured — no pixel regression) + SPIR-V drift-guard `test_octree_config_sdi_parity` **1/1** (generated GLSL's compiled 432 B / offsets == the aliased C++ struct). END-TO-END single-source proof: one canonical `[GpuStruct]` → C++ (alias) + GLSL (`#include`) agreeing at the SPIR-V level, rendering identically. Opus validation UNAVAILABLE (channel returning injected non-verdicts) — the live render + drift-guard ARE the authoritative empirical gates · 2026-07-02

**★ Phase C + cleanup COMPLETE ★** — the hand-written `Vixen::SVO::OctreeConfig` is fully retired;
C++ and GLSL consume the canonical-generated single source, and the superseded parallel
`Vixen.Codegen` was removed in `1b075391`.
...
Run via [[post-brainstorm-context-manager]] on VIXEN `feat/config-codegen`: **C1** (Task 1) Sonnet + Opus validator, controller-run full-build + test-suite + drift-guard gate; **C2** (Task 2) Sonnet + Opus validator, controller-run **live lavapipe render gate** (authoritative) + drift-guard. Phase C and the follow-on Cleanup are complete: the hand-written OctreeConfig is retired and the parallel `Vixen.Codegen` was removed in `1b075391`.
