...
**★ Phase C COMPLETE ★** — the hand-written `Vixen::SVO::OctreeConfig` is fully retired; C++ and GLSL both consume the canonical-generated single source. Remaining in the epic: the **Cleanup phase** (delete the superseded parallel `Vixen.Codegen` from VIXEN).
