...
| P3-H10 | hot recipe groups → low skip-mask words | CPU maps the SSBO, clears every word, sets hot-instance bits, and unmaps (`VIXEN/application/main/source/VulkanGraphApplication.cpp:752-766`; map/unmap implementation is `VIXEN/libraries/RenderGraph/src/Nodes/StorageBufferNode.cpp:134-146`). | every frame when bucketed dispatch is enabled | S, buffer-size dependent | **DIRECT** — preserve one contiguous mask write but use a persistent mapped per-frame slot; remove map/unmap and make publication match the frame ring. |
| P3-H11 | `RecipeRegistry` entries → bound-sphere SSBO | The CPU clears the whole table and rewrites center/radius/relaxation/threshold fields for every registered recipe (`VIXEN/application/main/source/VulkanGraphApplication.cpp:773-795`). | every frame when bucketed dispatch is enabled | M worst case; 32 B × table entries | **DIRECT** — cache the table by registry generation and write only changed entries through a persistent mapping; the struct already is the shader layout. |
| P3-H12 | hot groups → bucket-meta SSBO | The CPU maps the meta table and writes member count, rect origin, radius, and relaxation for each hot recipe (`VIXEN/application/main/source/VulkanGraphApplication.cpp:810-834`). | every frame when bucketed dispatch is enabled | S; 32 B × hot recipes | **DIRECT** — same persistent-mapped, dirty-entry publication as P3-H11; no format conversion is needed. |
...
| 2 | P3-H10/P3-H11/P3-H12/P4-H11: per-frame map/fill/unmap of masks and recipe tables | `DIRECT` through existing frame-indexed storage buffers; cache bound-sphere data by registry generation and publish dirty meta/mask ranges. |
...
| L4 | Recipe bucket tables map/unmap on every pre-tick | The three CPU writers use `MapForReadback`/`UnmapReadback` (`VIXEN/application/main/source/VulkanGraphApplication.cpp:752-834`). | **DIRECT** to persistent mapped frame slots; the current API name is readback-oriented even though these calls are CPU writes. |
