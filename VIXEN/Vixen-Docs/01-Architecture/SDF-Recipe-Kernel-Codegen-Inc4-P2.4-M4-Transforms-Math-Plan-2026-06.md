...
- Milestone M4c (Task 3): DONE · Yeroket `20e70b33` + VIXEN `4443649d` · Opus validator APPROVED · 2026-06-28
  - 27 value-math ops (opcodes 56-82): 16 unary + 6 binary + Lerp + Select + PositionChannel + Displacement + DistanceTo. CppMappingTables + HLSL MappingTables both got smoothstep/frac→fract/step/exp/log/log2 (+saturate). Value-stack dispatch (unary peek / binary pop / ternary). MathPow sign-preserving `pow(abs,d0)*sign`; Log/Log2 guarded `log(max(x,1e-30))`.
  - Self-tamper 17/17 front-loaded by impl; validator INDEPENDENTLY re-tampered 13 (operand-swap Sub/Div/Lerp/Select + identity-break Abs/Sign/Sqrt/Pow/Smoothstep/Log + leaf PositionChannel/Displacement/DistanceTo) → each failed ONLY its own test, 0 collateral. No toothless test.
  - ENUM audit: all 27 generated == canonical SDFOpCode by name+value, contiguous 56-82, no collisions; eval+emit dispatch by named `SdfOpCode::X` (only bare-ints are PositionChannel's 0/1/2/3 channel selector = canonical data0.x).
  - Parity 73/73, codegen/SPIR-V 9/9, render 9/9; live RenderDisplacement=92,832px (validator PNG read = bumpy/wavy solid, not smooth). No-regression all exact. Vendor diffs (.g.hpp/.g.hlsl/SdfOpCodes.g.h) empty.
  - CARRIED TO M4d (non-blocking): **N1** Select(81)+Displacement(74) are pure-scalar but hand-written inline in eval+emit (single-source duplication) → kernelize to `SdfCore_` in M4d. **N2** RenderDisplacement's only gate is `ASSERT_GT(px,10000)` — a smooth sphere also passes; strengthen (diff vs amp=0 baseline / silhouette-variance) in M4d (same teeth-class as the M3 carved-Subtract lesson). **N3** 4 pre-existing dotnet fails stay tracked (ChainDispatchIndirect + 3 RefKind/Blackboard; unrelated to SDF).
...
**Carried from M4c (address in M4d):**
- **N1 — kernelize Select+Displacement (single-source).** M4c shipped Select(81)=`cond>thr?a:b` and Displacement(74)=`sdf+disp*scale` as hand-written inline math in BOTH eval (C++) and emit (HLSL) — pure scalar, so this duplicates canonical in VIXEN. MOVE the math to generated kernels `SdfCore_Select(float cond,float a,float b,float thr)` and `SdfCore_Displacement(float sdf,float disp,float scale)`; VIXEN keeps the stack-arity dispatch but CALLS the kernel for the math, and the VIXEN inline math is deleted. Re-tamper to confirm teeth survive.
- **N2 — strengthen the displacement render gate (teeth).** `RenderDisplacement`'s only assertion is `ASSERT_GT(bodyPixels,10000)`, which a SMOOTH sphere also passes — the displacement is currently proven only by a human PNG read. Add a real gate: render an amp=0 baseline and `ASSERT` the displaced render's pixel set differs by > a threshold (or a silhouette-variance check). Same teeth-class as the M3 carved-Subtract lesson.

- [ ] **Step 1 (Yeroket):** add the float3 `SdfCore_Float3*` kernels + the **N1** `SdfCore_Select`/`SdfCore_Displacement` scalar kernels; mark the VM-control opcodes `[SdfCoreOp]`. Regen; dotnet green; g++-compile; HLSL no bare-int div. Commit + vendor (verbatim — diff empty).
