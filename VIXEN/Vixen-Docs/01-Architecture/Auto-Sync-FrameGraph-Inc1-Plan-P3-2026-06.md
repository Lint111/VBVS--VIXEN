...
- [x] **Milestone 3 — Task 3:** `ComputeDispatchNode` replays baked `entryBarriers` (`barrier2`); delete hand-rolled transitions. **Execution-changing → syncval gate is HANDS-ON.** Implementer: **Sonnet**, then syncval-verified. ✅
...
- **Milestone 3 (Task 3): DONE** · `ComputeDispatchNode` replays baked barriers as `barrier2` (WSI acquire/present transitions node-managed; inter-pass replay additive); v1 transitions deleted · commit `78ffbe56` · Opus code-review APPROVED after a WSI-fix (unconditional acquire + additive replay). **Surfaced + fixed TWO pre-existing prerequisites:** (a) the `synchronization2` device feature was never enabled → `barrier2` hit `VUID-…-03848`; fixed the proper way via the **capability graph** (query → register `DeviceFeature:synchronization2` → gate, hard-error if absent) — commit `fbde5c04`; (b) `UISelectionProviderNode` missing its self-registration line → app boot abort; fixed — commit `528cd41c`. **syncval: 0 validation/sync errors** on the windowed benchmark (the only prior error was the sync2 feature-gap, never a `SYNC-HAZARD`). Build green; unit tests green (frame_sync 10, tracker 23, node-reg 2). Pushed: `origin/feat/auto-sync-framegraph-p3` = `78ffbe56` · 2026-06-21.
  - **Caveats (separate pre-existing issues, NOT M3/auto-sync):** the windowed benchmark exits early (code 5) after render setup; the full app `VIXEN.exe` has further graph-wiring rot (`ray_size_bias` ConstantNode "output resource not allocated") — both predate auto-sync (the benchmark is the maintained render path) and are tracked as follow-ups, not P3 blockers.
