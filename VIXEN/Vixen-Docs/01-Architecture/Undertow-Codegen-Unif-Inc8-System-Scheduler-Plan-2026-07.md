...
- [x] **Milestone 2 (Task 2):** build + equivalence proof + retire. DONE + Opus-validated APPROVED,
  2026-07-13. **INCREMENT 8 COMPLETE.**
...
- Milestone 2 (Task 2, build + equivalence proof + retire): DONE (pending Opus validation) · 2026-07-13
  - **Yeroket side** (`feat/codegen-unif-inc8-system` off `feat/codegen-unif-inc7-effect` tip `c4691435`,
    commit `b71b84d4`): added `CompilationLoader.LoadSystemClasses` (mirrors `LoadActionClasses`/
    `LoadEffectClasses`); a new `SourceGenerator~/Transpiler/SystemRegistrationEmitter.cs` holding a
    VERBATIM port of `SystemScheduleSolver` (including the `PhaseRank` KEEP-IN-SYNC map, Kahn's topo-sort,
    UTSYS004/005/006) plus `SystemDecl` + the emitter logic ported line-for-line from
    `EmitRegisterSystems.cs`; a new `--system-cs` CLI branch in `Program.cs` (UTSYS002/UTSYS003 validated
    at the CLI layer, mirroring the Roslyn generator's pre-Emit checks in `SystemRegistrationGenerator.
    Initialize`). 11 ported/new NUnit tests in `SystemScheduleSolverTests.cs` (8 solver tests including a
    new synthetic `BeforeOnly_OverridesOrder`, since Milestone 1 found real data is 100% After/0% Before)
    + 6 in `SystemRegistrationEmitterEmitTests.cs` (converted from undertow's `EmitRegisterSystemsTests.cs`)
    — all pass.
  - **Equivalence proof**: ran a real `dotnet build` in the undertow worktree BEFORE retirement (captured
    actual Roslyn `RegisterSystems.g.cs`, 37 real sites). Ran the new `--system-cs` CLI against the SAME
    real `Undertow.Sim/Systems` source tree. Byte-diff (banner excluded, `diff` on the method bodies) —
    **0 differences**, byte-identical for all 37 real sites, including the 3 real linear chains
    (Knowledge/Integrate/Observe phases) with matching baked `Order` values. All 5 diagnostics
    (UTSYS002/003/004/005/006) verified together in one live CLI run against a synthetic malformed
    `Systems/Synthetic_BadSystems.cs` file added to a scratch copy of the schema dir — exit code 1, all
    5 diagnostics fired correctly (missing `ISystem`, missing `ICoarseSystem`, a 2-node cycle, a dangling
    `After` ref, a cross-phase `After` edge).
  - **Retirement** (undertow worktree `.claude/worktrees/codegen-unif-inc8-system`, commit `67282524`):
    deleted `SystemRegistrationGenerator.cs`, `EmitRegisterSystems.cs`, `SystemScheduleSolver.cs`.
    `SystemScheduleSolverTests.cs` + `EmitRegisterSystemsTests.cs` were PORTED to Yeroket (not just
    deleted) since a solver/emitter this load-bearing warranted keeping its own unit tests alongside the
    port. Checked in `core/src/Undertow.Sim/GeneratedSystems.g.cs` as an ordinary compiled file (banner
    restored to the original Roslyn `<auto-generated/>` header, matching Inc-7's precedent for
    checked-in generated files). `CodeModLoader.cs` confirmed byte-identical pre/post (md5
    `bc9b5256de67d16ea504a7c7958b0ab0` unchanged) — its SYSTEMS block's raw-Order-only, no-Before/After
    mod-load asymmetry is untouched, exactly as scoped.
  - **Full build + test**: `dotnet build core/Undertow.sln` — 0 errors, 0 warnings. `dotnet test
    core/Undertow.sln` — `Undertow.Core.Tests` 2921/2921 passed, `Undertow.Vixen.Host.Tests` 21/21 passed
    (2942 total, 0 failures).
  - No blockers.
  - **Opus validator (independent re-verification):** APPROVED. Rebuilt undertow at the pre-retirement
    commit from scratch to re-derive the byte-identical equivalence proof (real Roslyn body ==
    checked-in file body == new CLI body, all matching md5), independently confirmed the synthetic
    `BeforeOnly_OverridesOrder` test is genuine and non-vacuous (an inverted Order sort forced correct
    by `Before`), independently ran all 5 diagnostics live against its own synthetic malformed scenario,
    confirmed `CodeModLoader.cs`'s zero diff and its Order/Before-After asymmetry unchanged (not
    silently fixed), and independently re-ran the full test suite from a clean state (2942/2942
    matching). **Increment 8 (`[System]` schedule solver) COMPLETE.**
