...
- **M2 — Generic view-host + native Hud consumer (the decouple + live wire).** Tasks 3-5. Deliverable: `UIRenderNode` is view-agnostic (`IView` seam); a native `HudView` consumer in the main app calls `BindHudModel`; the live HUD renders through it. Includes the byte-exact real-GPU PNG gate. **✅ DONE.**
- **M3 — RML markup partial emit + compose + close-out.** Tasks 6-8. Deliverable: Yeroket `RmlMarkupEmitter` generates the data-binding partial; `hud.rml` composes it; `test_ui_hud_smoke` migrated; full gate green; docs closed. **✅ DONE.**
...
- **M2 (Tasks 3–5, generic host + HudView consumer + live gate): DONE** · commits `ad430c46` (T3 — `UIRenderNode` generic `IView` host) + `4913c608` (T4 — native `HudView` consumer + `Hud.g.h` moved to `application/main/include/Generated/`) + `dc369750` (T5 — wire + real-GPU capture gate) on `worktree-view-contract-inc2` · **node fully view-agnostic** (grep for HudFaction/HudEvent/SetHudView/factions_/… = ZERO; live render/composite/GPU-sync machinery byte-untouched) · `IView.h` = pure interface (`ModelName`/`Register`/`DocumentPath`); node has `SetView`/`MarkViewDirty` · `HudView : IView` (main app) owns storage of `Vixen::Views::HudFaction`/`HudEvent`, `Register()`→`BindHudModel`, faithful relocated `SetHudView` projection · **LIVE GATE `test_hud_render_capture` 3/3 on REAL GPU**: A/B-on-known-data — payload A (frame45: "Logistics(4)/acme 3.5/T1 raid") vs payload B (frame75: "Intel(1)/umbra 0.2/no events") differ by **9786 px** (of 250k; kMinHudDiffPixels=40, 245× margin), cross-run byte-deterministic (validator ran the `.bat` twice — all PNGs md5-identical), visually confirmed the generated `BindHudModel` drives every projection field · `test_view_hud_golden` 3/3 (incl. new `IViewSeamIsViewAgnostic`), `test_hud_view` 1/1 · Opus validator APPROVED (independent re-run of the live gate + all unit gates) · 2026-07-06.
  - **3 unplanned additions, all validator-confirmed justified/minimal/correct:** (a) **`HudViewBridge.{h,cpp}` + `HudFactionEventTypes.h`** — an ODR/ABI fix for a REAL collision: gaia vendors robin_hood **v3.11.5**, RmlUi **v3.9.0**, both under guard `ROBIN_HOOD_H_INCLUDED` → first-included wins per-TU → `Rml::DataTypeRegister` laid out 64 vs 56 bytes in gaia-touching TUs → null-deref crash. Fix = a gaia-free forwarding seam: `HudView.h`'s RmlUi include is confined to `HudView.cpp`+`HudViewBridge.cpp`; `VulkanGraphApplication`/`BuildRenderGraph` call `MakeHudView/DestroyHudView/WireHudView/PushHudView` only; `hudView_` is a raw `HudView*` (header forward-decls only; ctor/dtor via the bridge, since `unique_ptr` would need the complete type at the gaia-touching dtor site). (b) **`CaptureSwapchainToPng`** (new in `RenderTargetReadback.h`, +185/−0, editor's `CaptureRenderTargetToPng` byte-unchanged) — the HUD composites onto **`main_swapchain`** (SWAPCHAIN_PUBLIC slot 1), NOT `compute_render_target` (the plan's wrong assumption); the helper does the `PRESENT_SRC_KHR↔TRANSFER_SRC_OPTIMAL` round-trip a swapchain image needs. (c) **1-line shader fix** `BodyInstanceRayMarch.comp:634` — a genuinely PRE-EXISTING missing `stack` arg (10 args vs the 11-arg signature; present since before M1), which blocked the app from running at all; unrelated to view-contract but an enabling fix.
  - **MINOR NIT for M3 close-out sweep:** `VulkanGraphApplication.h`'s first added comment block says "Held by unique_ptr (constructed via MakeHudView()…)" but the member is actually a raw `HudView*` — the member's own second comment is correct; fix the stale phrase during M3.
  - **Also for M3 close-out:** validator did NOT re-run the editor's `test_editor_toggle_undo_capture` fresh (no PNGs staged in this worktree); its `CaptureRenderTargetToPng` path is byte-unchanged, but a fresh windowed run to re-verify is a residual sweep item.
- **M3 (Tasks 6–8, smoke migrate + markup emit + compose + close-out): DONE** · commits `a4d57fba` (T6, VIXEN — smoke migrate) + `1293dae0` (T7, Yeroket — RmlMarkupEmitter) + `b00ddfba` (T8, VIXEN — compose + close-out) on `worktree-view-contract-inc2` / `feat/view-contract-inc2` · **Opus validator APPROVED** (independent fresh re-run of every gate on real GPU: smoke 6/6, golden 3/3, hud_view 1/1, Yeroket 20/20, live 3/3 @ 9814px, both `--check` 0; compose-faithfulness confirmed — shell preserved verbatim, `recentChanged` addition justified; **independently confirmed KI-016 is pre-existing** — `git diff 68942244..HEAD` = ZERO editor/ActionStack/undo files across the whole increment) · **T6:** `test_ui_hud_smoke.cpp` repointed off the deleted `Nodes/UIRenderNode.h` surface onto `graph/HudView.h`; `S1b_SetHudViewListsResolveInRml` now actually drives a `Vixen::App::HudView` instance (`Register`+`SetHudView`) instead of hand-rolled mirror structs, `S1b_SetHudViewApiCompiles` type-checks `Vixen::App::HudFactionIn/HudEventIn` (the plan assumed `Vixen::RenderGraph::` namespacing — M2's actual gaia-ODR fix relocated them to `Vixen::App` via `HudFactionEventTypes.h`, adjusted accordingly); CMake adds `HudView.cpp` as a direct source (not the `VixenApp`/gaia-touching library) + `application/main/include` to the test target (mirrors `test_view_hud_golden`'s identical include-dir pattern) · **6/6 PASS** fresh Windows-side rebuild. **T7:** `RmlMarkupEmitter.cs` (sibling of `RmlDataModelEmitter`, same `Yeroket.KernelFramework.Codegen` namespace) emits a bare `{{}}`/`data-for` binding fragment in schema field order; `--view-markup <Name> --out-rml <path> [--check]` branch added to `Program.cs` mirroring `--view`; TDD'd (`RmlMarkupEmitterTests.cs` written failing first, confirmed compile-error FAIL, then implementation made it PASS) · **whole tool suite 20/20 PASS**, no regression; `SDFNodeGenerator.dll` churn reverted each run, Yeroket left clean. **T8:** generated `Hud.view.rml` (bare fragment: 4 scalars + `factions`/`events` `data-for` loops over all 6+2 row fields) via `--view-markup Hud`; confirmed this RmlUi build's `<template>`/`data-include` (`TemplateCache`/`XMLNodeHandlerTemplate`) is a whole-**document** `<body template="...">` composition mechanism, not a mid-tree fragment include — used the plan's marked-region fallback instead: `<!-- GEN:hud-bindings BEGIN/END -->` now wraps `hud.rml`'s existing data-bound region, with the hand-authored shell (`.clock`, `.lens-status`, `.section-label`, per-row classes, the `✓`/`·` ternary) preserved verbatim inside it. **Field-completeness audit found a real pre-existing gap:** the schema's `HudFaction.recentChanged` (the "juice" field — M2's `HudFactionEventTypes.h` comment calls it "drives the .changed CSS pulse") was never actually bound anywhere in `hud.rml`/`hud.rcss` — fixed in-scope by adding `data-class-changed="f.recentChanged"` to the faction row (matching the existing `.focused`/`.known`/`.in-lens` data-class convention) + a `.row.changed .col-value { color: #f0c060; }` rule in `hud.rcss`; every schema field is now bound exactly once. `view_hud_markup_check` CHECK=0 (stable regen) and `view_hud_check` CHECK=0 (re-verified) · CMake `view_hud_markup_check`/`view_hud_markup_regen` added mirroring `view_hud_check`/`_regen`; KI-015 warning text updated to mention `Hud.view.rml`. **Live gate re-run PASS**: fresh `VIXEN` rebuild + `main_hud_capture.bat` + `test_hud_render_capture` **3/3 PASS**, `wholeFrameDiffPixels(png45,png75)=9814` (up slightly from M2's 9786 — expected, since the newly-wired `.changed` tint now also differs between payload A [juice ON] and B [juice OFF]); **cross-run byte-determinism independently re-proven** by running the `.bat` twice and md5-comparing all 3 PNGs (identical both runs) — a stronger check than M2's original single-run same-file-reload determinism test. **Full no-regression sweep, all fresh Windows-side rebuilds:** `test_view_hud_golden` 3/3, `test_hud_view` 1/1, `test_ui_hud_smoke` 6/6, `test_hud_render_capture` 3/3, `view_hud_check` + `view_hud_markup_check` both CHECK=0.
  - **Sweep item 1 (stale comment) FIXED:** `VulkanGraphApplication.h`'s first `HudView`-related comment block said "Held by unique_ptr (constructed via MakeHudView()…)" while the member (`hudView_`) is actually a raw `HudView*` (its own adjacent comment was already correct) — corrected the stale phrase to "Held by a raw pointer".
  - **Sweep item 2 (fresh editor-gate re-verify): RAN, FOUND A REAL PRE-EXISTING BUG, UNRELATED TO VIEW CONTRACT.** Built `vixen_editor` fresh, ran `VIXEN/temp/run_editor_script.bat` (exit 0, produced all 4 PNGs), built + ran `test_editor_toggle_undo_capture` fresh: **FAILED** — `boreDiffPixels(png5,png45)=1024` confirms the toggle visibly changed the render (PASS half), but `png75.rgb != png5.rgb` (undo@60 did NOT restore the baseline). md5 comparison of all 4 captures shows `editor_capture_45`/`_75`/`_105` are byte-IDENTICAL to each other and differ from `_5` — i.e., `rt_.Undo()` at frame 60 had no visible effect at all; the render stayed in the post-toggle state through the rest of the run. This is the FIRST time this gate has actually been re-run fresh since M2 (M2's own log noted it was never re-verified, only assumed-safe from an additive-only readback-path diff); zero changes exist anywhere in `application/editor/`, node sources, or the undo/ActionStack path across M1-M3 of this increment (confirmed via `git status`/`git diff` scoped to those paths) — this is a genuine, previously-latent regression in the editor's undo path, not caused by View Contract Inc-2. Flagging for a new Known-Issues entry / separate investigation; NOT fixed here (out of this increment's scope, and the plan's Global Constraints explicitly wall off the editor/ActionStack surface). **→ Logged as KI-016** (`9f91af94`; bisect target `79786a66..main`); the M3 validator independently confirmed the pre-existing attribution.
