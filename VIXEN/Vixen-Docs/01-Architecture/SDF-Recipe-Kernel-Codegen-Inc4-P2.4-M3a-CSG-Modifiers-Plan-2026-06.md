...
- [ ] **Step 3 (live render):** Extend `test_procedural_recipe_render.cpp` with `RenderSubtractBoxSphere`: recipe `Subtract(Box, Sphere)` (Box halfExtents ~0.7, Sphere at origin-ish r~0.55 so it carves a visible cavity in the box face) → write `/tmp/glsl_sdf_m3a_subtract.png`; assert non-trivial body px. Run ICD-only. **Validator reads the PNG** — must show a box with a concave spherical bite (proves non-commutative Subtract on the GPU path matches the CPU).
