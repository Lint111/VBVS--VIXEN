...
- R1 (uniform registry + verb retirement): IMPLEMENTED, commit 8ff9f20d (5 files; offline suite 11/11 exes green Windows-side; AppFlow.g.h sha f7a1d870 unchanged; grep gate clean in libraries/, 5 expected editor call sites remain for R5). **VALIDATION INCOMPLETE — PIPELINE PAUSED 2026-07-10: Opus validator died on the account monthly spend limit.** Pending validation items: (1) rule on the dropped ActionApplied/ActionUndone/ActionRedone Publish() calls (implementer verified no TEST asserts them; non-test consumers unswept); (2) run test_appflow_editor_toggle_render WSL-side (needs a software Vulkan device absent on native Windows; no WSL build dir exists in this worktree yet — first configure ~500s); (3) confirm the temporarily-broken editor target (5 deleted-verb calls, R5 scope) is contained to the editor executable. Resume = re-dispatch the R1 validator with exactly these three items + the standard diff/suite/tree checks, then R2.
