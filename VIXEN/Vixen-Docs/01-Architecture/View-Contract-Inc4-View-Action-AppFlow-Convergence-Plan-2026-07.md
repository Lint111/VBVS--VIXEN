...
- R3 (D16 Data->View-noun target): DONE - Yeroket 17ddbfe5 ([FlowDataTarget] + model/builder validation + AppFlowTargetException + --view-schema + DLL) + VIXEN e4de3e14 (Data=6 + "bodyCount" target + CMake --view-schema + regen sha c7e082d2 -> b63b2b35 + golden 7/7). Opus validator: code+gates APPROVED (guard-invariant HOLDS: Data-less schemas byte-identical; ISet-for-IReadOnlySet required by netstandard2.0; negative path re-proven exit 2). D16 realization note: generation-time cross-schema check via --view-schema IS the correct form of "compile-checked" (separate Roslyn compilations make C# nameof cross-refs impossible).
- **MERGE-SEQUENCING CONSTRAINT (discovered R3, validator-confirmed):** from R3 onward the committed AppFlow.g.h requires the `feat/appflow-reframe-slice` Yeroket branch's emitter — a build tree cached to Yeroket MAIN stale-diffs appflow_check by exactly the Data lines. **The Yeroket reframe branch must merge to Yeroket main BEFORE (or together with) this VIXEN branch merging/gating anywhere.** Same will apply to R4's --callable-cpp/callables_check.
- R4 (D12 logic-transplant walking skeleton): DONE - Yeroket 752709ff (--callable-cpp CLI + LoadKernelCallables) + e4f5c50c (fix: uint->uint32_t in CppMappingTables.TypeMap; DLL recommitted) / VIXEN c0a22bb9 + 53029b0d (AppFlowCallables.cs schema + vendored AppFlowCallables.g.hpp sha 989b65e4 + callables_check/regen pair). Opus validator APPROVED: artifact==tool-output byte-identical; smoke-compiled AND RUN (self-inverse proven for m<64,i<32); AppFlow.g.h b63b2b35 + SdfCoreKernels.g.hpp 9919d0bb unchanged; 42/42+230/230; tamper 0->1->0. FIX-LOOP: first vendoring emitted bare `uint` (invalid C++) — root-caused to a CppMappingTables.TypeMap gap (same latent-mapping-gap family as the kernel-unification bool->uint8_t find). CAVEAT (validator Rule 1, Minor): C# masks shift counts, C++ shift>=32 is UB — transplanted applyToggle is behavior-identical only for index<32 (fine for layer masks; R5 adds the caveat comment to AppFlowCallables.cs; masking wrapper NOT wanted).
- R6 (live gate + close-out): DONE - VIXEN 65a4816f (test_editor_toggle_undo_capture.cpp reworked + EditorApplication.h/.cpp state-dump + run_editor_script.bat + new finding doc R6-Editor-Render-Mask-Invisible-Finding-2026-07.md). **KEY OUTCOME — the live gate did its job: it exposed that there is NO render/sync/descriptor/AppFlow bug.** The original gate premise (capture_75==capture_5 byte-exact to prove undo restores the render) asserted an INVISIBLE-BY-DESIGN quantity: a max-effort Opus GPU-boundary investigation (impl-r6 3 hypotheses + debug-r6) proved via GPU-memory checksums + a real shader-output tap that the layer mask reaches the shader CORRECT+FRESH (A/B/A across toggle/undo/redo) at every layer (CPU state, shell-cache derive/upload to both slots, GPU buffer contents, descriptor write into the bound VkDescriptorSet, shader read) — but the editor renders the mask-INVARIANT coarse mip-fallback (`OctreeConfig.brickResident==0`, because RequestBrickResidency is only driven by camera movement and scripted runs never move the camera) at an orbit camera where the CSG cut delta is ~6px → the mask has ~0 visible effect. The one visible change (d3db→449027 at "first toggle") was the non-resident→resident transition, NOT the mask (decisive control: two consecutive same-direction toggles post-residency render byte-identical). Engine/shader files (BodyOctreeSceneNode/ComputeDispatchNode/DescriptorSetNode/RenderTargetNode/BodyInstanceRayMarch.comp) proven correct + left PRISTINE (all instrumentation reverted). **Gate REWORKED (user decision) to assert what's real+editor-observable, 3 tests all PASS live on WSL/Dozen:** (1) ToggleUndoRedoStateTrailThroughWindowedRun — parses the running editor's [EDITOR/state] log to assert mask trail 7→3→7→3 + undo/redo depths (the real proof undo/redo work through the windowed registry-dispatch path); (2) FirstEditReachesRenderPipeline — one-shot smoke check capture_5!=capture_45 (=1024px), explicitly the RESIDENCY transition not the mask; (3) BackButtonReachesReturnInRunningEditor — afterBack==Editing(0). Removed the invisible-by-design pixel round-trip (png75==png5/png105==png45) + kMinBoreDiffPixels. FOLLOW-UP (separate content/LOD matter, NOT an AppFlow defect): make the editor body request brick residency so the SDF march runs and mask edits become visible. Two Opus workers showed exemplary discipline: FOUR hypotheses, each disproven with hard instrumented evidence rather than a forced fix. Gates: 48/48 AppFlow (12 exes) green Windows-side; reworked test+editor compile+link clean; byte-guards 989b65e4 + b63b2b35 unchanged. Opus validator: [pending].
- R5 (editor pure consumer + swap transplanted body): DONE - VIXEN 96a45012 (6 files: EditorApplication.h/.cpp, KeyMap.h new, AppFlowCallables.cs caveat comment, AppFlow/tests/CMakeLists.txt + test_keychord.cpp). ParseLayerToggleId + the ToggleLayer verb DELETED; 5 self-contained handlers registered in LoadDocument (reload-guarded via handlersRegistered_): ToggleLayer routes through Stack().Dispatch (undoable) + LIVE Generated::applyToggle(mask,idx) [D12 swap]; Undo/Redo via Stack(); Save bare side-effect; Return via NavPop(). UI click drain -> DispatchBySelector(clickedId); keys -> edge-detected DispatchByKey (S/Ctrl+Z/Ctrl+Y/Esc) via new KeyMap.h (GLFW keycode -> typed KeyId, no GLFW dep); PreTick injector rewired onto the same registry path. **Opus validator (val-r5) APPROVED 2026-07-10:** 6-file scope correct, byte-guards EXACT (AppFlowCallables.g.hpp 989b65e4, AppFlow.g.h b63b2b35 — unchanged, R5 is C++-only), purity greps empty (no deleted-verb calls, no ParseLayerToggleId), ran test_keychord.exe independently 3/3, transplant byte-identical for valid indices, tree coherent (parent e80b59a9, clean). Gates: 12/12 AppFlow suites green (48 tests) + vixen_editor.exe links clean. Implementer-found behavior fix (in scope): Save dispatches {S, KeyMod::None} not live-ctrl mods, else Ctrl+S would silently stop saving (regression vs pre-Inc-4). Implementer ALSO ran the WSL render gate early: test_appflow_editor_toggle_render PASSED (Dozen/D3D12 RTX 3060, boreDiffPixels=6400, byte-exact undo) — de-risks R6. Two non-blocking caveats (walking-skeleton scope): new SetMask(applyToggle) path drops old i>=count_ bounds guard (diverges only for out-of-range idx real selectors never produce; index<32 caveat documented); plan Files-line said "Modify AppFlowRuntime.cpp" but its Step-4 body prescribes the editor-handler swap (implementer followed Step-4, 6-file set is spec-correct). NOTE: impl-r5 hit a transient spend-limit mid-milestone; controller drove build/gate/commit, then impl-r5 recovered and committed the IDENTICAL SHA 96a45012 (convergence confirmed).
