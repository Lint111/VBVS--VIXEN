...
3. `KernelDispatch` is the shared TDD foundation for RenderGraph and the application, and remains omitted from `VIXEN_EXPORTED_TARGETS` (`libraries/KernelDispatch/include/KernelDispatch/Abi.h:7-20`; `cmake/VixenInstall.cmake:46-74`) until the SDK ABI/hash gate is approved.
...
5. Existing concurrency is fragmented across the remaining SVO TBB recursion and other private work paths. These islands have different admission, shutdown, exception, ordering, and worker-count rules; the former EventBus thread bridge, shader worker pool, cache `std::async` launches, slot-task launches, and scene-body futures have been retired in favor of `KernelDispatch`.
...
|---|---|---|---|---|
| `KernelDispatch` ABI and dispatcher. | `Abi.h` declares stages, access, profiles, backends, and handles; `Dispatcher.h` implements per-element, stage-chain, per-item-hazard, and precomputed-wave entry points (`libraries/KernelDispatch/include/KernelDispatch/Abi.h:7-20,33-189`; `Dispatcher.h:34-90,93-169,200-419`). RenderGraph lowers frame and slot work to `TaskExecutor`; the application scene bake also uses the injected MainCacher executor. | `CpuInline` uses one worker and `CpuTbb` maps items/waves to a bounded TBB task arena; future GPU/transfer values are declared but not implemented. | The header explicitly requires per-element results to be invariant across worker counts (`Dispatcher.h:17-20,43-47`). Access-derived RAW/WAW/WAR edges order conflicting stages/items. | This is the shared foundation. Its public dispatcher wrappers remain synchronous, while `TaskExecutor` owns the graph's frame waves and MainCacher's blocking lane. No consumer should instantiate a private executor for production work. |
...
| `SlotTaskManager`. | `NodeInstance` owns one manager and has **one production call path** to `ExecuteParallel` (`libraries/RenderGraph/include/Core/NodeInstance.h:1152`; `src/Core/NodeInstance.cpp:400-426`). The implementation batches task contexts and requires the graph's injected `MainCacher::GetTaskExecutor()` (`src/Core/SlotTask.cpp:133-267`). | Each call uses the shared executor for each budget-sized batch, with a barrier after every batch. Unowned nodes use the existing sequential path because they have no graph-owned executor. | Results remain stored by task index; stop-token cancellation, ordered failure handling, and the 1/2/N worker-count gate are preserved. | **Migrated in T-035.** The manager no longer constructs a private `TaskExecutor`; executor ownership is explicit at the API and supplied by `MainCacher`. |
...
| EventBus `WorkerThreadBridge<T>`. | **Retired in T-029 (2026-09-04).** The declaring header and its stale EventBus build-list entry were the only live source artifacts; the prior shader-builder consumer had already migrated to `KernelDispatch`'s blocking lane. | The deleted bridge owned one FIFO worker thread and published results directly from it. | Its FIFO work order was stable, but publication timing relative to main-thread message processing was not; shutdown could leave queued work unprocessed. | **Decommissioned.** Future async EventBus work should submit to the shared executor and publish completed results at a deterministic main-thread/frame boundary. |
...
| Scene-specific parallel body bake. | One cache-miss block in `BuildRenderGraph.cpp` submits **eight** indexed `VirtualTask`s through `renderGraph->GetMainCacher().GetTaskExecutor()` and gathers results in canonical body order (`application/main/source/graph/BuildRenderGraph.cpp:5919-5990`). | Shared `KernelDispatch::TaskExecutor` admission executes independent bodies, while a process-wide Gaia allocator mutex serializes the allocation-sensitive part. | Indexed result storage and downstream assignment remain stable. The manually scoped global lock and worker-logging exclusion preserve the live safety boundaries discovered during T-031. | **Migrated in T-031.** Keep the shared executor and allocator serial boundary; do not reintroduce per-body futures or a scene-owned pool. |
...
| `DispatchMaterializationBatch`. | The census found **zero production consumers** and only test consumers; it now lowers an owned request vector to one typed `KernelDispatch::Stage` (`libraries/SVO/src/BulkMaterialization.cpp`). | The shared dispatcher owns worker admission and barriers; indexed result slots preserve input order. | Tests cover terminal success/failure/cancellation, upload-ready page output, and 1-vs-N canonical-hash parity. | **Migrated in T-030.** It is an SVO adapter over `KernelDispatch::RunPerElementStage`; no materialization-specific queue or TBB arena remains. |
...
## 17. T-035 implementation record — 2026-09-07

Delivered:

- `SlotTaskManager::ExecuteParallel` now receives an explicit shared `KernelDispatch::TaskExecutor`.
- `NodeInstance::ExecuteTasks` supplies `owningGraph->GetMainCacher().GetTaskExecutor()`, so production
  slot-task batches share the graph's executor ownership instead of constructing a private executor.
- Unowned nodes retain a safe sequential fallback because no graph-owned executor exists; budget batching,
  stop-token cancellation, indexed result storage, and post-batch ordering are unchanged.
- Slot-task tests now exercise the same explicit-executor contract; the implementation source census has
  no consumer-local `KernelDispatch::TaskExecutor` construction.
