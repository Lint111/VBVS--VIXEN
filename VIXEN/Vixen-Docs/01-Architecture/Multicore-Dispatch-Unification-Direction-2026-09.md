...
Not delivered in M1 or the decision #6 slice: graph-sibling scheduling, parallel Vulkan command recording, frame pipelining, the two-way kernel/node bridge, concurrency-island retirement beyond `SlotTaskManager`, `InstanceGroup` retirement, managed/modder vocabulary changes, SDK installation/export changes, or any M2–M6 work.
