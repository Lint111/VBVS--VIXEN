...
- Milestone 2 (Tasks 3–4, fan-in proof): DONE · commits `ed114fa4` (generic `ComputeStageNode`), `bf5d4aef` (fan-in demo+shaders+env), `578b10d0` (CORE scheduler fix) · Opus validator APPROVED (core blast-radius SAFE, verified live). **The live gate found a deeper bug than framed:** the scheduler baked spurious timeline edges for UNTYPED passthrough/metadata accesses (via `ProvisionalKind`) → a non-submitting metadata node became a phantom edge endpoint whose timeline value never signaled → consumer deadlock → present `VUID-03268` → black window. **Pure fix:** only accesses with a declared `AccessKind` participate in timelines (an edge needs BOTH endpoints declared; edges 41→2 for the fan-in); `ProvisionalKind` removed. Makes the live composite correct BY CONSTRUCTION, not topology luck. **Fan-in live-proven:** the SPLIT renders (left=A red/green, right=B blue/cyan, clean seam) timeline-ONLY (no binary handoff between producers/consumer), 0 syncval — both timeline waits honoured. **Live-composite regression RUN: 29 groups/0 timeline edges, renders 12 frames, 0 syncval/VUID/hazard/device-lost, no hang** (compute→UI now carried by the M1 additive binary net until M3 declares UI's `AccessKind` → the edge re-bakes). Tests: scheduler 11/11 (+`UntypedPassthrough_ProducesNoEdge`), timeline 2/2, passgroup 3/3. Generic `ComputeStageNode` does NOT touch the live `ComputeDispatchNode`. **M3 setup:** declare UI's swapchain `AccessKind` (edge re-bakes) + remove `leaveImageInGeneral` + the binary handoff → timeline-only live composite. · 2026-06-22
- Milestone 3 (Task 5 + live gate Task 6): DONE · commits `58c9e6ee` (declare UI `AccessKind::ColorAttachmentWriteGeneral` + drop binary handoff) + `87aa268f` (fix) · **Live gate caught a layout bug** (`VUID-vkCmdDraw-09600` + `VUID-VkPresentInfoKHR-01430`): dropping the binary handoff CONNECTION also removed the only graph dependency ordering compute before UI → topo sort INVERTED (UI before compute) → the timeline edge baked backwards → compute ran last (left GENERAL → present saw non-PRESENT_SRC) + UI's render pass ran on an UNDEFINED image. **Fix:** re-add the `renderComplete→compositeWait` CONNECTION purely as an ORDERING edge (the binary semaphore stays inert — compute doesn't signal it in composite, UI doesn't wait it; timeline-only ordering preserved). **Live re-verified: 0×09600, 0×01430, 0 syncval, `1 edge` compute→UI, ~117s run, no hang.** Scheduler 11/11, barrier_types 5/5. The live composite is now ordered compute→UI by the timeline edge alone. **Separate finding (NOT P5b):** the standalone app's RmlUi HUD overlay doesn't render visibly (hud.rml defines persistent elements, but nothing shows) — a pre-existing UI-rendering bug, orthogonal to auto-sync (the composite PASS runs: the scene shows through the UI LOAD). · 2026-06-22
- Milestone 4 (Task 7 + live gate Task 8): DONE · commit `5b37a8af` · WSI timeline-law assert (no timeline `SyncEdge` may target the acquire group or originate from the present group on the swapchain resource; the legal compute→UI edge passes) + unit test (scheduler 12/12) + cosmetic comment scrubs. KEPT the `COMPOSITE_WAIT_SEMAPHORE` ordering edge + `leaveImageInGeneral` (no behaviour change). **Final live regression: live composite renders timeline-only, 29 groups/1 edge, 0 syncval, no assert-abort, ~76s run, "Exiting normally" (no hang).** Tests: scheduler 12/12, timeline 2/2, barrier_types 5/5. **P5b COMPLETE (M1–M4) — the live composite's compute→UI handoff is now timeline-only at 0 syncval; the multi-submit fan-in proves the timeline; the scheduler bakes only declared-hazard edges.** · 2026-06-22
