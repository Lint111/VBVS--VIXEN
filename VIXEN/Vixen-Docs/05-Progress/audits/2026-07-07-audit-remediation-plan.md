...
- FINAL REVIEW (whole branch, 2026-07-08): READY — cross-milestone interactions clean (throws never unwind under the submit mutex; no double-log; no lock inversion); 56/56 branch-new tests pass; ctest 56/2080 = baseline; integration = trivial rebase onto origin/claude/wsl-build-portability (one docs commit ahead).

## Follow-up resolution (2026-07-10) — CPU SVO cluster + parity band

The V0-baseline's "57 pre-existing failures, separate follow-up recommended" turned out to be REAL code bugs, not flakes, and are now fixed. Engine `origin/main` fast-forwarded to `3defcd8d` (branch `svo-clusters`); undertow pin bumped to it.

- **EntityBrickViewTest ~22 SEGFAULTs** → `setEntity` was bounded by `m_voxelsPerBrick`, but query-mode ctors carry an EMPTY entity span, so every write scribbled past a null span. Bound by the actual span (no-op in query modes). Suite converted to span-backed construction; query-mode no-op regression test added. 33/33.
- **Root-anchored traversal domain** → the octree converges to a shallower root than `maxLevels` for clustered sparse content; traversal sized its domain/scale from `maxLevels` and rays missed real voxels. Added `Octree::rootDepth` + `rootShortfall()`/`effectiveLevels()`/`effectiveWorldMax()` (full resolution for frame-spanning trees; shrunk only for genuinely shallow roots). Fixes PartialBlockUpdate.LockForRenderingAPI + BodyOctree.CoarseLodGivesCoarserShell.
- **`glm::clamp(ivec3)` -O3 miscompile** in `updateBlock`/`removeBlock` → integer division; **placement-new UB** on a reference-member type → orphan-and-repoint; **`BrickView::getLinearIndex`** → delegates to the Morton `coordsToStorageIndex` so 3D writes and index reads agree.
- **CornellBox min-face DDA** → the brick-DDA hit's local→world scale was identity (`worldSize/(worldMax-worldMin)`), leaking local voxel-grid coords through unscaled; a min-face floor hit at local-y=1 reported world-y=1 instead of `1*(10/16)=0.625`. Fix: `scale = worldSize/(bricksPerAxis*brickSideLength)`, snapped to exact 1.0 when `worldSize == gridResolution` (integer-grid body/shell convention) so that path stays bit-exact and doesn't regress BrickDDAStepConsistency's `>=` boundary assert. CornellBox 9/9.
- **ShaderCacheManagerTest** (per-test-name + per-pid temp dirs, green under `-j`) and **SwapChainNodeTest** (stale 7→8 input assertion) also fixed.
- **Parity band coverage gap (V5 follow-up)** → `GpuParity.MirrorBoundaryEpsilonMatchesShaderSource` now pins mirror `boundary_epsilon == ESVOTraversal.glsl` at the SOURCE level (negative-checked: fails on 1e-2), and `BoundaryEpsilonBandNearFaceEntry` adds the (1e-4, 1e-2) ray-regime coverage.

CPU SVO cluster 129/129; CornellBox 9/9. Remaining ctest failures are the GPU-render env SEGFAULT baseline (BodyInstanceRayMarch/RecipePool/MipFallback/EditorDocument/RecipeAuthoringGate — need a Windows `.bat` PNG prerun), plus ONE real pre-existing bug: `TierCrossingLodResidencyTest.NonResidentChildNeverCrossesResidentChildDoes` (main's own Tiered-ESVO Inc2-M4 tier-crossing bug — resident child renders 0 magenta px; verified identical on clean main). That is the last non-environmental follow-up.
